...
export class OpenRouterService {
  ...
  private async checkRateLimit(): Promise<void> {
    ...
    if (!canProceed) {
      ...
      throw new RateLimitError(
        `Rate limit exceeded. Please wait ${Math.ceil(timeUntilReset / 1000)} seconds.`
      );
    }
  }
  ...
  private validateConfiguration(config: Partial<OpenRouterConfiguration>): void {
    ...
    if (config.modelParameters) {
      if (typeof config.modelParameters.temperature !== 'number' || 
          config.modelParameters.temperature < 0 || 
          config.modelParameters.temperature > 1) {
        throw new Error('Temperature must be a number between 0 and 1');
      }
      if (typeof config.modelParameters.max_tokens !== 'number' || 
          config.modelParameters.max_tokens <= 0) {
